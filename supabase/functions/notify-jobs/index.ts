...
Deno.serve(async (req) => {
  ...
  if (pre) return pre;
  if (req.method !== "POST") return json({ error: "GC-JOBS-010: método no permitido" }, 405);
  ...
  try {
    ...
    const { data: tenants, error: errTenants } = await supabase
      ...
      .select("id, nombre")
      .eq("activo", true);
    if (errTenants) return json({ error: "GC-JOBS-013: no se pudieron leer tenants" }, 500);
    ...
    for (const tenant of tenants ?? []) {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/push-notifications`, {
        ...
        body: JSON.stringify({
          ...
          cuerpo: `Tienes ${[...porAsesor.values()].reduce((a, b) => a + b, 0)} visita(s) programada(s) para mañana ${maniana}.`,
